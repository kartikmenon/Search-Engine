...
int append(List *list, WebPage *page);

/*
 Inserts a new element between the two WebPages already in the doubly linked List.
 Input: A pointer to the linked list, a pointer to the WebPage struct before the insertion, and a pointer to the WebPage struct after the insertion.
 Output: An int > 0 if successful, = 0 if unsuccessful.
 */

int insertList(List *list, WebPage *before, WebPage *after);
